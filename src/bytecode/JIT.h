...
namespace iris::bytecode {
    ...
    public:
        ...
        LoopFunc compileSimpleLoop() {
            ...
            emit8(0x8B);
            emit8(0x02); // mov eax, [rdx]
            emit8(0x41);
            emit8(0x3B);
            emit8(0xC0); // cmp eax, r8d
            emit8(0x7D);
            emit8(0x0A); // jge end (+10 bytes)

            size_t loop_start = offset;
            emit8(0xFF);
            emit8(0x01); // inc dword ptr [rcx]
            emit8(0xFF);
            emit8(0xC0); // inc eax
            emit8(0x41);
            emit8(0x3B);
            emit8(0xC0); // cmp eax, r8d
            emit8(0x7C);
            emit8(0xF7); // jl start_loop (-9 bytes)

            emit8(0x89);
            emit8(0x02); // mov [rdx], eax
            emit8(0xC3); // ret
