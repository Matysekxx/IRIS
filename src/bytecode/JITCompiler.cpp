    ...
    for (size_t i = 0; i < chunk.code.size(); ++i) {
        ...
        switch (op) {
            ...
            case OpCode::OP_LOADBOOL: { x86::Gp regA = (A < 5) ? vRegs[A] : x86::rax; if (A >= 5) a.mov(regA, x86::qword_ptr(rBase, A * 8));
                emitRelease(regA); a.mov(regA, boolTag | (B != 0 ? 1 : 0)); if (A >= 5) a.mov(x86::qword_ptr(rBase, A * 8), regA); break; }
...
JITFunc JITCompiler::compileTrace(Trace& trace, void* functions_ptr, void* native_functions) {
    ...
    std::vector<x86::Gp> vRegs = { x86::r12, x86::r13, x86::r14, x86::rbp, x86::rbx };
    std::vector<bool> isUnboxed(5, false);
    uint64_t intTag = iris::core::Value::QNAN | iris::core::Value::TAG_INT;
    uint64_t boolTag = iris::core::Value::QNAN | iris::core::Value::TAG_BOOL;
    uint64_t nullTag = iris::core::Value::QNAN | iris::core::Value::TAG_NULL;
    ...
    for(int i = 0; i < 5; i++) {
        a.mov(x86::rax, x86::qword_ptr(rBase, i * 8));
        a.mov(x86::r10, x86::rax); a.shr(x86::r10, 48); a.cmp(x86::r10w, intPrefix);
        Label notInt = a.new_label(); a.jne(notInt);
        a.mov(vRegs[i].r32(), x86::eax); isUnboxed[i] = true;
        Label done = a.new_label(); a.jmp(done);
        a.bind(notInt); a.mov(vRegs[i], x86::rax);
        a.bind(done);
    }
    ...
    for (const auto& entry : trace.entries) { uint32_t instr = entry.instr; OpCode op = decodeOp(instr); uint8_t A = decodeA(instr); uint8_t B = decodeB(instr); uint8_t C = decodeC(instr);
        switch (op) {
            ...
            case OpCode::OP_LOADK: {
                if (A < 5) {
                    a.mov(x86::rax, (uint64_t)instr & 0xFFFF); a.mov(x86::rax, x86::qword_ptr(constants, x86::rax, 3));
                    if (isUnboxed[A]) {
                        a.mov(x86::r11, x86::rax); a.shr(x86::r11, 48); a.cmp(x86::r11w, intPrefix);
                        emitGuard(x86::CondCode::kEqual, entry.pc); a.mov(vRegs[A].r32(), x86::eax);
                    } else { a.mov(vRegs[A], x86::rax); }
            ...
            case OpCode::OP_MOVE: {
                if (A < 5 && B < 5) {
                    if (isUnboxed[A] && isUnboxed[B]) { a.mov(vRegs[A].r32(), vRegs[B].r32()); }
                         ...
                         a.mov(x86::r11, vRegs[B]); a.shr(x86::r11, 48); a.cmp(x86::r11w, intPrefix);
                         emitGuard(x86::CondCode::kEqual, entry.pc); a.mov(vRegs[A].r32(), vRegs[B].r32());
                    } else {
            ...
            case OpCode::OP_INC: {
                if (A < 5 && isUnboxed[A]) { a.inc(vRegs[A].r32()); }
                else { a.mov(x86::rax, (uint64_t)entry.pc); a.jmp(sideExitTrampoline); }
                break;
            }
            case OpCode::OP_ADDI: { 
                if (A < 5 && B < 5 && isUnboxed[A] && isUnboxed[B]) {
                    a.add(vRegs[B].r32(), (int8_t)C); a.mov(vRegs[A].r32(), vRegs[B].r32());
                } else { a.mov(x86::rax, (uint64_t)entry.pc); a.jmp(sideExitTrampoline); }
                break;
            }
            case OpCode::OP_LT_INT: {
                x86::Gp valB = x86::eax; x86::Gp valC = x86::edx;
                if (B < 5 && isUnboxed[B]) a.mov(valB, vRegs[B].r32());
                else { 
                    if (B < 5) a.mov(x86::r10, vRegs[B]); else a.mov(x86::r10, x86::qword_ptr(rBase, B * 8));
                    a.mov(x86::r11, x86::r10); a.shr(x86::r11, 48); a.cmp(x86::r11w, intPrefix); emitGuard(x86::CondCode::kEqual, entry.pc); a.mov(valB, x86::r10d); 
                }
                if (C < 5 && isUnboxed[C]) a.mov(valC, vRegs[C].r32());
                else { 
                    if (C < 5) a.mov(x86::r10, vRegs[C]); else a.mov(x86::r10, x86::qword_ptr(rBase, C * 8));
                    a.mov(x86::r11, x86::r10); a.shr(x86::r11, 48); a.cmp(x86::r11w, intPrefix); emitGuard(x86::CondCode::kEqual, entry.pc); a.mov(valC, x86::r10d); 
                }
                a.cmp(valB, valC); a.setl(x86::r10b); a.movzx(x86::r10d, x86::r10b);
                if (A < 5) {
                    if (isUnboxed[A]) { a.mov(vRegs[A].r32(), x86::r10d); }
                    else { a.mov(vRegs[A], boolTag); a.or_(vRegs[A], x86::r10); }
                } else { a.mov(x86::rax, boolTag); a.or_(x86::rax, x86::r10); a.mov(x86::qword_ptr(rBase, A * 8), x86::rax); }
                break;
            }
            case OpCode::OP_JMPF: {
                x86::Gp val;
                if (A < 5) {
                    if (isUnboxed[A]) { a.mov(x86::r11d, vRegs[A].r32()); a.and_(x86::r11d, 1); val = x86::r11d; }
                    else { a.mov(x86::r11, vRegs[A]); a.and_(x86::r11, 1); val = x86::r11d; }
                } else { a.mov(x86::rax, x86::qword_ptr(rBase, A * 8)); a.and_(x86::rax, 1); val = x86::eax; }
    ...
    a.add(x86::rsp, 8); a.pop(x86::rbx); a.pop(x86::rbp); a.pop(x86::rsi); a.pop(x86::rdi); a.pop(x86::r15); a.pop(x86::r14); a.pop(x86::r13); a.pop(x86::r12); a.ret();
    JITFunc func; if (rt.add(&func, &code) != kErrorOk) return nullptr;
    return func;
}
